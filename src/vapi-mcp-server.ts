...
class VapiMCPServer {
  ...
  /**
   * Get ALL tool definitions (no filtering for this deployment)
   * This server exposes ALL 239 tools - use for ElevenLabs or advanced integrations
   */
  private getFilteredToolDefinitions() {
    // NO FILTERING - Return all tools for full functionality
    const allTools = this.getAllToolDefinitions();
    
    // Only exclude delete_contact for security
    const filteredTools = allTools.filter(tool => tool.name !== 'delete_contact');
    
    console.log(`[MCP ALL] Exposing ALL tools: ${filteredTools.length}/${allTools.length} (delete_contact excluded for security)`);
    console.log(`[MCP ALL] This deployment is for full tool access - NOT optimized for Vapi AI`);
